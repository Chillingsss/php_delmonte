...
function getExamResult($json)
{
  ...
  $sql = "SELECT c.status_name, b.app_candId FROM tblapplicationstatus a
          INNER JOIN tblapplications b ON a.appS_appId = b.app_id
          INNER JOIN tblstatus c ON a.appS_statusId = c.status_id
          WHERE b.app_candId = :cand_id AND a.appS_statusId IN (10,9)";
  $stmt = $conn->prepare($sql);
  $stmt->bindParam(':cand_id', $candId, PDO::PARAM_INT);
  $stmt->execute();
  return $stmt->rowCount() > 0 ? json_encode($stmt->fetchAll(PDO::FETCH_ASSOC)) : 0;
